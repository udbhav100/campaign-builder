...
import { Campaign, AdCopy, Estimates, ToneType, BusinessSnapshot } from './types';
...
const KEYWORD_MAPPINGS: Record<string, { type: string, keywords: string[], emoji: string, competitors: string[], differentiate: string[] }> = {
  'salon': { 
    type: 'Beauty Salon', 
...
export const generateSuggestionsFromUrl = (url: string, businessName?: string, city?: string, tone: ToneType = 'Neutral') => {
  ...
  const score = (str: string) => str.length % 3;
...
export const calculateEstimates = (sliderValue: number, objective: string): Estimates => {
